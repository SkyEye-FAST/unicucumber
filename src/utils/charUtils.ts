...
const isCJKCodePointValue = (code: number): boolean =>
  ...
  (code >= 0x323b0 && code <= 0x3347f) || // CJK Extension J
  (code >= 0x31c0 && code <= 0x31ef) || // CJK Strokes
  (code >= 0xf900 && code <= 0xfaff) || // CJK Compatibility Ideographs
  (code >= 0x2f800 && code <= 0x2fa1f) // CJK Compatibility Ideographs Supplement

export const isCJKCodePoint = (value: number): boolean =>
  isUnicodeScalarValue(value) && isCJKCodePointValue(value)

export const isCJKChar: CJKCharFunction = function (char) {
  const code = char.codePointAt(0)
  if (code === undefined) return false
  return isCJKCodePointValue(code)
}
